...
class HeaderCalc(Node):
    ...
    def get_header(self):
        ...
        now.sec = 0 
        now.nanosec = 0 
        fake_header = Header( stamp=now, frame_id=self.frame_id)
        '''
...
class SynchronizerMinTick(ApproximateTimeSynchronizer):
    ...
    def __init__(self, fs, queue_size: int, slop: float, min_tick: float):
        ...
        self.min_tick = rclpy.Duration.from_sec(min_tick)
        self.last_tick = rclpy.Time()
